...
symbol_value_map = {'bp': 256, 'B': 1024, 'Seq': 1024, 'H': 256, 'D': 100, 'DF': 400, 'DI': 200, 'DO': 100}
...
def prepare_templates(symbolic_dir_):
    ...
    tensors = Tensor.parse_records(os.path.join(symbolic_dir_, "processed_graphs", "transformer_24.csv"))
    offload_strategy = OffloadStrategy.create_blank(tensors)
    offload_strategy.to_records(os.path.join(symbolic_dir_, "offload_strategy", "blank", "transformer_24.csv"))
    return


def sample_symbol_value_map(i_, symbolic_dir_, et_dir_):
    offload_strategy = OffloadStrategy.parse_records(os.path.join(symbolic_dir_, "offload_strategy", "blank", "transformer_24.csv"))
...
def convert(symbolic_dir_, et_dir_, symbol_value_map_):
    ...
    for i in range(num_samples):
        ret.append(pool.apply_async(convert_task, (i, symbolic_dir_, et_dir_, symbol_value_map_)))
    for i in ret:
        i.get()
    return True
...
if __name__ == '__main__':
    ...
    ret = convert(symbolic_dir_=symbolic_dir, et_dir_=et_dir, symbol_value_map_=symbol_value_map)
