...
def transformer_stack(root='sharding_spreadsheets/dp', visualize=False):
    mha_fwd = Tensor.parse_records(os.path.join(root, 'graphs/multiHeadAttentionFwd.csv'))
    mha_bwd = Tensor.parse_records(os.path.join(root, 'graphs/multiHeadAttentionBwd.csv'))
    ffn_fwd = Tensor.parse_records(os.path.join(root, 'graphs/feedForwardNetworkFwd.csv'))
    ffn_bwd = Tensor.parse_records(os.path.join(root, 'graphs/feedForwardNetworkBwd.csv'))
...
def transformer_stacks(num_stacks, root='sharding_spreadsheets/dp', visualize=False):
    ...
    if visualize:
        Tensor.visualize(fwd, os.path.join(root, f'visualization/stack_{num_stacks}_fwd_stacks'))
        Tensor.visualize(bwd, os.path.join(root, f'visualization/stack_{num_stacks}_bwd_stacks'))
    Tensor.to_records(fwd, os.path.join(root, f'processed_graphs/stack{num_stacks}Fwd.csv'))
    Tensor.to_records(bwd, os.path.join(root, f'processed_graphs/stack{num_stacks}Bwd.csv'))


def transformer(num_stacks, root='sharding_spreadsheets/dp', visualize=False):
    if not os.path.exists(os.path.join(root, f'processed_graphs/stack{num_stacks}Fwd.csv')):
        transformer_stacks(num_stacks, root=root, visualize=visualize)
    assert os.path.exists(os.path.join(root, f'processed_graphs/stack{num_stacks}Bwd.csv'))
    stacks_fwd = Tensor.parse_records(os.path.join(root, f'processed_graphs/stack{num_stacks}Fwd.csv'))
    stacks_bwd = Tensor.parse_records(os.path.join(root, f'processed_graphs/stack{num_stacks}Bwd.csv'))
    ...
    if visualize:
        Tensor.visualize(fwd, os.path.join(root, f'visualization/transformer_{num_stacks}_fwd'))
        Tensor.visualize(bwd, os.path.join(root, f'visualization/transformer_{num_stacks}_bwd'))
    Tensor.to_records(fwd, os.path.join(root, f'processed_graphs/transformer_{num_stacks}_fwd.csv'))
    Tensor.to_records(bwd, os.path.join(root, f'processed_graphs/transformer_{num_stacks}_bwd.csv'))
    ...
    if visualize:
        ...
        pass
    Tensor.to_records(loop, os.path.join(root, f"processed_graphs/transformer_{num_stacks}.csv"))
