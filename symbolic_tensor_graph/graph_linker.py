...
class GraphLinker:
    ...
    def prefix_graph(self, cmd):
        def _parse_terms(cmd):
            ...
            graph_name = graph_name.string[
                graph_name.start() : graph_name.end()
            ].strip()
            prefix_name = (
                prefix_name.string[prefix_name.start() : prefix_name.end()]
                .replace(";", "")
                .strip()
            )
    ...
    def link_graph_impl(graphs, link_node):
        ...
        for from_, to_ in link_node.items():
            ...
            to_.direct_output_shape, to_.direct_output_hidden = (
                from_.direct_output_shape,
                from_.direct_output_hidden,
            )
